...
class TwoHypothesisTest(PhysicsModel):
    def __init__(self): 
        ...
        self.ftwIncluded = False
        self.ftwFloating = False
        self.poiMap  = []
        self.pois    = {}
        self.verbose = False
        self.altSignal  = "ALT"
    ...
    def getYieldScale(self,bin,process):
        ...
        elif self.ftwIncluded:
            ret = self.sigNorms[isAlt]
            if isAlt: 
                ret+= self.sigNormstw["tW" in process]
    ...
    def setPhysicsOptions(self,physOptions):
        for po in physOptions:
            if po == "ftwIncluded":
                self.ftwIncluded = True
                self.muFloating = True
                print 'tW fraction in alternative signal will be consiered with overall signal strength floating'
            if po == "ftwFloating":
                self.ftwIncluded = True
                self.ftwFloating = True
                self.muFloating = True
                print 'tW fraction will be floating as well as the signal strength'
            if po == "muAsPOI": 
                self.muAsPOI = True
                self.muFloating = True
    ...
    def doParametersOfInterest(self):
        ...
        if self.muFloating: 
            if self.pois:
            	...
            	if self.ftwIncluded:
                    if self.ftwFloating: 
                        self.modelBuilder.doVar("ftw[0,0,1]")
                    else: 
                        self.modelBuilder.doVar("ftw[0]");
                    self.modelBuilder.factory_("expr::r_times_x_times_ftw(\"@0*@1\", r_times_x, ftw)")
                    self.modelBuilder.factory_("expr::r_times_x_times_not_ftw(\"@0*(1-@1)\", r_times_x, ftw)")
                    self.sigNormstW = {True:'_times_ftW',False:'_times_not_ftw'}
